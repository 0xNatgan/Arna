...
\begin{itemize}
    \item \textbf{Le routeur (Gating Network)} :
          ...
          Il existe aussi des variantes plus hybrides avec un une selection d'experts en focntion d'un seuil.
...
\begin{itemize}
    ...
    \item \textbf{Les couches Mixture of Experts (MoE Layers)} :

          Une couche MoE remplace une couche dense classique par un ensemble d'expertes en parallèle et
          Chaque expert est un sous modèle spécialisé, la forme et lacomplexité des experts peuvent varier en fonction de la tâche principale et de la conception du MoE
          mais leur rôle est le même : traiter une partie spécifique du problème.
          Dans le cas ou tout les experts sont de même type, on parle de MoE homogène, sinon on parle de MoE hétérogène.
          Les MoEhomogènesp
          Les couches MoE remplacent les couches de réseau \textit{feed-forward} (FFN) denses.
          Les couches MoE contiennent un certain nombre d'\textit{experts} (par exemple 8), où chaque expert est un réseau de neurones.
          En pratique, les experts sont des FFN, mais ils peuvent également être des réseaux plus complexes, voire un MoE lui-même, conduisant à des \textbf{MoE hiérarchiques}.

    \item \textbf{Réseau de routage (Gating Network)} :
          Il détermine quels tokens sont envoyés à quels experts.
          Par exemple, le token ``More'' peut être envoyé au deuxième expert, tandis que le token ``Parameters'' est envoyé au premier réseau.
          Comme nous l'explorerons plus tard, il est possible d'envoyer un token à plusieurs experts simultanément.
          La décision de \textbf{comment router un token vers un expert} est l'une des décisions majeures lors de la conception d'un MoE, le routeur est composé de paramètres appris et est pré-entraîné en même temps que le reste du réseau.
\end{itemize}
